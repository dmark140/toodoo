  <tr>
      <td class="border-y border-r min-w-[50px]">
          <div>
              <form action="{{ route('noteStatus.put', $id) }}" method="POST" class=" flex w-full p-0 m-0">
                  @method('put')
                  @csrf
                  <button type="post">
                      <div class="border ">
      ...
      <td class="border-y pl-8 w-full">
          <div class="flex">

              <form action="{{ route('note.put', $id) }}" method="POST" class=" flex w-full p-0 m-0">
                  @method('put')
                  @csrf
                  @if ($completed == true)
                      <input class="w-full hover:cursor-cell line-through" name="note" value="{{ $note }}" />
                  @else
                      <input class="w-full hover:cursor-cell " name="note" value="{{ $note }}" />
                  @endif
              ...
              <form action="{{ route('note.destroy', $id) }}" method="POST" class="p-0 m-0">
                  @method('delete')
                  @csrf
                  <button type="post">
